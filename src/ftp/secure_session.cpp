...
namespace ftps
{
    ...
    {
        std::make_shared<ftps::secure_session>(std::move(m_control_socket), m_ssl_context, is_implicit, m_session_id)
            ->start();
    }
    ...
    secure_session::secure_session(boost::asio::ip::tcp::socket socket, boost::asio::ssl::context &ssl_context,
                                   bool is_implicit, std::string session_id)
        : m_ssl_context(ssl_context), m_control_socket(std::move(socket), m_ssl_context),
          m_data_socket_acceptor(socket.get_executor()), m_data_socket(socket.get_executor(), m_ssl_context),
          m_buffer(MESSAGE_BUFFER_SIZE), m_receive_buffer(RECEIVE_BUFFER_SIZE)
    {
    ...
    void secure_session::data_directory_listing()
    {
    ...
    void secure_session::data_send_file()
    {
